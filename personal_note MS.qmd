...
-   The direct effect of past values on the current value - C
-   The total correlation between two points in time
-   The indirect effect of past values on the current value
-   The correlation between two variables, removing the effect of intervening variables - C
:::
...
-   A variable that influences both the cause and effect - C
...
-   The series has a constant mean and variance over time - C
...
-   The number of lagged forecast errors in the model - C
...
-   It becomes slower due to the increased computation of distances - C
...
-   To allow for a certain degree of misclassification in the training data - C
...
timetk::plot_time_series(
  ...
  .smooth = FALSE,
  .interactive = TRUE,
  .title = "Closing Prices of Five Major Canadian Banks"
)
```
...
The rsq metric for the fit of the arima model to the testing data is: **S**
:::
...
1.  Regressions that appear to lead to unbiased estimates of the causal effect are: lin_YV, lin_YW

2.  Valid adjustment sets for the data used in this question are: {V}, {W}, {V,W}
:::
...
splits <- spam_data %>% 
  initial_split(spam_data, prop = 0.75, strata = type)
...
#
# Example recipe using step_fourier for weekly seasonality
weekly_rec <- recipe(sales ~ date, data = training_data) %>%
  step_timeseries_signature(date) %>%     # add date-related features
  step_fourier(date, K = 2, period = 52)  # 52 weeks = yearly seasonal cycle
```
:::
