...
start <- matrix(c(1, 0, 0, 0), nrow = 1)

# distribution after 6 months
dist6 <- start %*% P6

# sum of probabilities of non-churn states (states 1–3)
prob_non_churn_6 <- sum(dist6[1, 1:3])
prob_non_churn_6
```
...
x       <- 1
lambda  <- 20
samples <- c(x, rep(NA, n - 1))  # to store samples

for (t in 2:n) {
  # proposal step: +/- 1 with prob 1/2 when x >= 1, or 1 when x = 0
  if (x >= 1) {
    if (runif(1) < 0.5) {
      y <- x + 1
    } else {
  ...
  # compute acceptance probability using the alpha() function
  a <- alpha(y, x, lambda)
...
theory_q     <- qpois(probs, lambda)

tibble::tibble(
  prob                = probs,
  sample_quantile     = as.numeric(sample_q),
  theoretical_quantile = theory_q
)
```
:::
