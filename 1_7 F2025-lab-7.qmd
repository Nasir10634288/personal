...
-   DAG_1
    -   open paths from D (treatment) to Y (outcome): D -\> A -\> Y
-   DAG_2
    -   open paths from D (treatment) to Y (outcome): D -\> E -\> Y
-   DAG_3
    -   open paths from D (treatment) to Y (outcome): "D -\> Y", "D \<- B -\> Y"
-   DAG_4
    -   open paths from D (treatment) to Y (outcome): "D -\> Y", "D \<- B \<- C -\> Y"
:::
...
net_data_wts <- propensity_model |>
  broom::augment(
    newdata = causalworkshop::net_data, 
    type.predict = "response") |>
  dplyr::mutate(
    ip_wts =             # mutate 1
      dplyr::case_when(
        net ~ 1/.fitted, # .fitted is the predicted probability of being treated
        TRUE ~ 1/(1-.fitted)),
    
    wts =                # mutate 2
      propensity::wt_ate(.fitted, net),
...
net_data_wts %>%  
dplyr::select(net, ip_wts, wts, `ip_wts = wts`)
...
smd_dat <- tidysmd::tidy_smd(split_data,
  c(income, health, temperature),
  .group = smpl,
) 
smd_dat

#Compare random vs non-random SMDs & plot the differences
smd_dat |> 
  dplyr::mutate(samples = "random") |>
  dplyr::bind_rows(
  tidysmd::tidy_smd(
    causalworkshop::net_data,
    c(income, health, temperature),
...
# Step 1 create debiasing model================================================
    # the de-biasing step predicts the treatment with all variables **except**      the treatment, How much of credit_limit can be predicted by wage and credit     scores?
    # To isolate the part of credit_limit that DOES NOT come from confounders.
...
# adding resuals to the main data table=========================================
...
# Step 2 create the de-noising model============================================
...
#
# Step 3 create the de-noising model============================================
...
lm(
  wt82_71~ qsmk + 
    sex + race + age+I(age^2) + education + #Demomgraphics 
    smokeintensity+I(smokeintensity^2) + smokeyrs+I(smokeyrs^2) + #Intensity
    exercise + active + #lifestyle
    wt71+I(wt71^2),  #baseline health
...
For a simple IPW model, we have two modeling steps.

First, we fit a propensity score model, which predicts the probability that you received a treatment or exposure (here, that a participant quit smoking). We use this model to calculate inverse probability weights--1 / your probability of treatment. Then, in the

second step, we use this weights in the outcome model, which estimates the effect of exposure on the outcome (here, the effect of quitting smoking on gaining weight).
...
$$ \hat{\rho} = \frac{\text{Cov}(Z, Y)}{\text{Var}(Z)}, \quad  \hat{\pi} = \frac{\text{Cov}(Z, D)}{\text{Var}(Z)}. $$

The IV (Wald) estimator is the ratio of these slopes:

$$ \hat{\beta}^{IV}  = \frac{\hat{\rho}}{\hat{\pi}}  = \frac{\frac{\text{Cov}(Z, Y)}{\text{Var}(Z)}}{\frac{\text{Cov}(Z, D)}{\text{Var}(Z)}}  = \frac{\text{Cov}(Z, Y)}{\text{Cov}(Z, D)}. $$
...
$$ E[Z(Y - \beta D)] = 0  \;\Rightarrow\; \text{Cov}(Z, Y) - \beta\,\text{Cov}(Z, D) = 0. $$
...
reduced_form <- lm(log(quantity) ~ shipping_cost, data = ecom_data)
